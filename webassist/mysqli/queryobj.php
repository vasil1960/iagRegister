		...
		case "update":
		  ...
		  for ($x=0; $x<sizeof($this->ParamColumns); $x++) {
		    if (!($this->ParamDefaults[$x] == "WA_IGNORE" && $this->ParamValues[$x] === "")) {
			  ...
			  $colPlaceholder = "?";
			  if ($this->ParamDefaults[$x] == "WA_DEFAULT" && $this->ParamValues[$x] == "") $colPlaceholder = "DEFAULT";
		...
		case "insert":
		  ...
		  for ($x=0; $x<sizeof($this->ParamColumns); $x++) {
		    if (!($this->ParamDefaults[$x] == "WA_IGNORE" && $this->ParamValues[$x] === "")) {
			  ...
			  if (strpos($columnRef,"`") == false && strpos($columnRef,"(") == false) $columnRef = '`'.$columnRef.'`';
              $Columns .= $columnRef;
			  if ($this->ParamDefaults[$x] == "WA_DEFAULT" && $this->ParamValues[$x] == "") {
				  $Values .= "DEFAULT";
  ...
  public function getEncryptedVal($val) {
	...
	private function getParams($index) {
	  ...
	  for ($x=0; $x<sizeof($this->ParamValues); $x++) {
		...
		if (!(($paramVal === "" || $paramVal === null) && ($this->ParamDefaults[$x] == "WA_DEFAULT" || $this->ParamDefaults[$x] == "WA_IGNORE" || $this->ParamDefaults[$x] == "WA_TIMESTAMP"))) {
		  $useParams[] = $paramVal;
